...
def _cron_binary() -> str:
    """Best runnable command for cron, as an absolute path (cron has a minimal
    PATH and starts in $HOME, so we never rely on PATH or the working dir).
...
@click.option("--output-dir", type=click.Path(), default=None)
@click.pass_context
def install_cron(ctx, input_file, every, output_dir):
    """Print a cron line to paste into your crontab (Linux only; does not edit it)."""
    if every < 2:
    ...
    # Cron runs in $HOME, so the config file (which holds the [notify] block) is
    # only found if we point at it with an absolute path. `--config` is a
    # group-level option, so it goes before the `scrape` subcommand.
    ...
    cmd = f"{binary} {config_opt}scrape --input-file {in_abs} --output-dir {out_abs} --quiet"

    click.echo("# Add this to your crontab (run `crontab -e`). Linux only.")
    click.echo("# Set MAILTO at the top of the crontab to receive non-zero-exit mail.")
    if cfg.notify.enabled:
        env_name = cfg.notify.smtp.password_env or "NEXTDOOR_WATCHER_SMTP_PASSWORD"
        click.echo(
            f"# SMTP notifications are enabled: make ${env_name} available by sourcing"
        )
        click.echo("# an env file (chmod 600) at the start of the line, as shown below.")
        click.echo(f"*/{every} * * * * . $HOME/.nextdoor-watcher.env; "
                   f"sleep $((RANDOM \\% 60)); {cmd} "
                   f">> {out_abs / 'logs' / 'cron.out'} 2>&1")
    else:
        click.echo(f"*/{every} * * * * sleep $((RANDOM \\% 60)); {cmd} "
                   f">> {out_abs / 'logs' / 'cron.out'} 2>&1")

    click.echo("\n# Belt-and-suspenders variant using flock:")
    flock_prefix = ". $HOME/.nextdoor-watcher.env; " if cfg.notify.enabled else ""
    click.echo(
        f"*/{every} * * * * flock -n /tmp/nextdoor.flock -c "
        f"'{flock_prefix}{cmd}'"
    )
