...
def _build_wrapper(binary: str, config_opt: str, in_abs: Path, out_abs: Path) -> str:
    """A bash wrapper that runs one scrape inside WSL. Windows Task Scheduler
    invokes it via wsl.exe. It sources the SMTP env file and `exec`s the watcher
    so its exit code becomes the task's 'Last Run Result'."""
    return (
        "#!/usr/bin/env bash\n"
        ...
        "sleep $((RANDOM % 60))\n"
        "\n"
        f"exec {binary} {config_opt}scrape \\\n"
        f"    --input-file {in_abs} \\\n"
...
def install_schedule(ctx, input_file, every, output_dir, task_name):
    ...
    wrapper_path.write_text(_build_wrapper(binary, config_opt, in_abs, out_abs), encoding="utf-8")
    try:
